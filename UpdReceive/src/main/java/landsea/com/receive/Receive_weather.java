 ...
 */
public class Receive_weather {
    public static void main(String[] args)throws IOException{
        String str_send = "Hello UDPclient";
        byte[] buf = new byte[160000];
        //服务端在8811端口监听接收到的数据
        DatagramSocket ds = new DatagramSocket(8812);
        //接收从客户端发送过来的数据
        ...
        while(f){
            ...
            String json_receive = new String(dp_receive.getData(),0,dp_receive.getLength());
            System.out.println(json_receive);
            JSONObject jsonObject = JSONObject.parseObject(json_receive);
            System.out.println(new WeatherSave().save(jsonObject));
            //数据发动到客户端的9000端口
